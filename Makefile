...
run-dev: ## run ezark in dev mode (with local mongo and nodejs/nodemon and without docker)
...
docker-run-debug: ## run ezark in debug mode with dockerized nodejs and mongodb process
	@NODE_ENV=development docker-compose -f ./docker-compose.debug.yml up --force-recreate

docker-run-dev: ## run ezark in dev mode with dockerized nodejs/nodemon and mongodb process
	@NODE_ENV=development docker-compose -f ./docker-compose.dev.yml up --force-recreate

docker-run-prod: ## run ezark in production mode with the full dockerized image (see docker-build)
...
docker-stop-prod: ## stop ezark production daemon
...
lint: ## to check the coding rules
	@./node_modules/.bin/eslint *.js heartbeats/ helpers/ loaders/ views/assets/ components/
